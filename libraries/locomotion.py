...
class Locomotion:
	...
	def control(self, direction, angle=None):
		...
		if direction == "forward": # ie if we are not turning
			...
			while True:
				with self.local.imu_angle_lock:
					start_angle = round(self.local.imu_angle, 2)
		...
		while True:
			...
			print("start_angle: ", start_angle, "current angle: ", round(self.local.lr_imu_angle, 3), "p$
			"distance: ", round(distance, 2), "counterFL: ", self.local.counterFL.value, "counterBR: ", \
			self.local.counterBR.value)

			if self.local.lr_imu_angle - start_angle >= max_diff: # this mean heading is toward the right
				print("leaning right")
				pin31 = loco.duty - left_adjustment
				pin37 = loco.duty + right_adjustment
				loco.drive([pin31, 0, 0, pin37]) # speed up right wheels
			elif self.local.lr_imu_angle - start_angle <= -max_diff: # if robot is leaning towards left.$
				print("leaning left")
				pin31 = loco.duty  + left_adjustment
				pin37 = loco.duty - right_adjustment
				loco.drive([pin31, 0, 0, pin37]) # speed up right wheels
				distance = self.local.tick_2_distance(self.local.counterFL) # check how far we have traveled
